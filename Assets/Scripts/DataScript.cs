...
public class DataScript : ScriptableObject
{
    ...
    int level;
    [SerializeField]
    List<string> data, topics;
    [SerializeField] 
    List<WordInfo> words;
    [SerializeField] 
    List<TestInfo> results = new List<TestInfo>();
    [SerializeField] 
    string[] testTopics = new string[4] { "1", "1", "1", "1" };
    [SerializeField]
    int[] itemIndex = new int[8];
    [SerializeField]
    float[] scrollbarValue = new float[8] { 1, 1, 1, 1, 1, 1, 1, 1 };
    ...
    void LoadPrefabs()
    {
        mainButton = Resources.Load<GameObject>("Prefabs/MainButton")
            .GetComponent<Button>();
        mainToggle = Resources.Load<GameObject>("Prefabs/MainToggle")
            .GetComponent<Toggle>();
    }
